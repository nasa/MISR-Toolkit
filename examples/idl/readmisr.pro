...
;   print,'map.nline=',map.nline
;   print,'sf=',sf
;   print,'FLIPPED l = ',l

   print,"point: ", i
;   print," (IDL DEVICE l,s) = ", l, s
...
PRO getw, w, h, base_id, win_id, map, sf, buf, fieldname
   ...
   WIDGET_CONTROL, base_id, SET_UVALUE={counter:0L,map:map,sf:sf,buf:buf,fieldname:fieldname}
...
bigbuf = rebin(buf,map.nsample*sf,map.nline*sf,/sample)
...
getw, map.nsample*sf, map.nline*sf, base_id, win_id, map, sf, buf, fieldname

win = win_id

if (disptype eq 0) then begin
    tv,bytscl(nbuf),/order
endif else begin
    tv,hist_equal(nbuf),/order
endelse

print, "Minimum data value = ",strtrim(min(buf),2)
print, "Maximum data value = ",strtrim(max(buf),2)
help,buf

status = mtk_ls_to_latlon(map, 0, 0, ulclat, ulclon)
status = mtk_ls_to_latlon(map, map.nline-1, map.nsample-1, $
                          lrclat, lrclon)
