...
/*****************************************************************************
  ...
  Input:
          ...
          classType   - If the new aggregate node is an object node, this
...

{
  ...
  if (kind == KA_OBJECT && classType != NULL)
    {
      string = (char *) malloc (strlen (classType)+1);
      if (string != NULL)
        {
          node->objClass = strcpy (string, classType);
        }
    }
  else
    {
...

{
  ...
  if(base_node->comment != NULL)
  {
  	free (base_node->comment);
	base_node->comment = NULL;
  }
  if(base_node->objClass != NULL)
  {
  	free (base_node->objClass);
	base_node->objClass = NULL;
  }
...

{
  ...
  node_copy = NewAggregate (NULL, base_node->kind,
                            base_node->name, base_node->objClass);
  if (node_copy == NULL)
    {
