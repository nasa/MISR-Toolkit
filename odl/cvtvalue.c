/*****************************************************************************
  ...
  Author:  Randy Davis, University of Colorado LASP
           Carol S. W. Tsai / Applied Research Corporation
  ...
  Modified code in order the data in UTC DATETIME format can be retrieved by
  user using the function PGS_MET_SetAttr() from the memory when metadata file
  was initialized or function PGS_MET_GetPCAttr() from the HDF Metadata file or 
  non-HDF Metadata file.(CSWT)
  Added code to set valid flag defined to check whether year is outside
  expected range or not to be 0 if it is in false case(CSWT) 
  Modified code in order the data in UTC DATETIME format with Zulu indicator or not
  can be written in a correct format to be retrieved (CSWT) (This changing is for
  ECSed15136 about the date_time values are too long)

  PDN Modified code so as to refuse 2 digit (or less) years for Y2K compliance.

  AT Modified code so as to set errno correctly in ODLConvertInteger and
     ODLConvertReal.
*****************************************************************************/
#include <limits.h>
#include <ctype.h>
...
VALUE_DATA ODLConvertInteger (char vtext[], int vlength)
{
  ...
  if (sp == NULL)
	...
	/* Used strtol instead of atol to get errno = ERANGE and value
	   equal LONG_MAX or LONG_MIN in case of overflow or underflow */

      base =10;
      item.format = 0;
      item.value.integer.number = strtol (vtext, NULL, base);
    }
  else
    {
  ...
  if (((item.value.integer.number == LONG_MAX) || 
       (item.value.integer.number == LONG_MIN)) && (errno == ERANGE))
    {
      sprintf (warning,
...
/* >>>>>>>>>>>>>>>>>>>>>>>>>> END OF CN CHANGES >>>>>>>>>>>>>>>>>>>>>>>>> */
  ...
  if (year < 100)
    {
       ODLPrintWarning ("Year value is < 100 and not valid.");
       item->valid = 0;
    }
  ...
  if (item->valid)
    {
      ...
      if (nparts == 2)
      {
          ...
          item->value.date_time.month = NULL;
          item->value.date_time.day   = NULL;
      }
      else
      {
          item->value.date_time.year  = (short) year;
  ...
  if (item->valid)
    {
      ...
      { 
         ...
         item->value.date_time.seconds     = NULL;
         item->value.date_time.nanoseconds = NULL;       
      } 
      else
      {
         item->value.date_time.hours       = (char) hours;
