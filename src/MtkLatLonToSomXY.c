...
int main( int argc, char *argv[] ) {
  ...
  status = MtkLatLonToSomXY(argr.path, argr.lat_dd, argr.lon_dd, &som_x, &som_y);
...
void usage(char *func) {
  fprintf(stderr, "\nUsage: %s <--help> | <--path=path> [<--dd=lat_dd,lon_dd> | <--rad=lat_r,lon_r> | <--dms=lat_dms,lon_dms>]\n\n",func);
  ...
  fprintf(stderr, "         MtkLatLonToSomXY --path=1 --rad=1.444098,-0.057778\n");
  fprintf(stderr, "         MtkLatLonToSomXY --path=1 --dms=82044026.490000,-3018037.650000\n");
}
...
int process_args(int argc, char *argv[], argr_type *argr) {
  ...
  while ((ch = getopt_long(argc, argv, "p:d:r:m:h", longopts, NULL)) != -1) {
