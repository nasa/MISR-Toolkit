...
typedef struct {
  ...
  char outfile[200];		/* Output ENVI filename */
...
int main( int argc, char *argv[] ) {
  ...
  status = MtkWriteEnviFile(argr.outfile, dbuf, mapinfo,
			    argr.infile, argr.grid, argr.field);
  if (status) {
    status_code = status;
    MTK_ERR_MSG_JUMP("MtkWriteEnviFile failed!");
  }
...
void usage(char *func) {
  ...
  fprintf(stderr, "       --envifilename=file is the output envi file.\n\n");
...
int process_args(int argc, char *argv[], argr_type *argr) {
  ...
  static struct option longopts[] = {
    ...
    { "envifilename",                required_argument, 0, 'b' },
