...
int main( int argc, char *argv[] ) {
  ...
  status = MtkPathTimeRangeToOrbitList(argr.path,argr.start,argr.end,
                                       &num_orbits,&orbit_list);
  if (status) {
    status_code = status;
    MTK_ERR_MSG_JUMP("MtkPathTimeRangeToOrbitList Failed.\n");
  }
...
void usage(char *func) {
  fprintf(stderr, "\nUsage: %s <--help> | <--path=path> <--start=YYYY-MM-DDThh:mm:ssZ> <--end=YYYY-MM-DDThh:mm:ssZ>\n\n",func);

  fprintf(stderr, "Where: --path=path is the path number\n");
  fprintf(stderr, "       --start=YYYY-MM-DDThh:mm:ssZ\n");
...
int process_args(int argc, char *argv[], argr_type *argr) {
  ...
  while ((ch = getopt_long(argc, argv, "p:s:e:h", longopts, NULL)) != -1) {
