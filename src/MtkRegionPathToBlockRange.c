...
int main( int argc, char *argv[] ) {
  ...
  status = MtkRegionPathToBlockRange(argr.region,argr.path,&start_block,&end_block);
  if (status) {
    status_code = status;
    MTK_ERR_MSG_JUMP("MtkRegionPathToBlockRange failed!");
  }
...
int process_args(int argc, char *argv[], argr_type *argr) {
  ...
  while ((ch = getopt_long(argc, argv, "p:u:e:l:h",
         longopts, NULL)) != -1) {
